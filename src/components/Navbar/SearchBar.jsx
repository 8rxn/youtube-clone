...
const SearchBar = () => {
  ...
  { width > 500 &&
    <form className='rounded-3xl flex items-center text-sm ' onSubmit={(e)=>{e.preventDefault(); if(location!="/"){navigate("/")}  }} >
      <input placeholder="Enter 3 or more characters to Search" id="search-input" autoComplete="off" type="text" value={searchText} onChange={(e)=>{setSearchText(e.target.value)}} className='rounded-l-3xl h-full w-[30vw]    px-4 rounded-r-none border-none mr-[2px] text-white placeholder:white/50 placeholder:italic bg-[#121212] ' name='search'></input>
...

  {
    ...
    {toggleSearch &&  <form className=' flex items-center z-30 absolute left-2 rounded-xl border-white border   top-2 bg-[#121212] h-10 w-[95vw] mr-2 text-sm' onSubmit={(e)=>{e.preventDefault();}} >
      <input placeholder="Enter 3 or more characters to Search" id="search-input" autoComplete="off" type="text" value={searchText} onChange={(e)=>{setSearchText(e.target.value)}} className='rounded-xl h-full w-[90%] px-4  rounded-r-none border-none mr-[2px] text-white placeholder:white/50 placeholder:italic bg-[#121212] ' name='search'></input>
